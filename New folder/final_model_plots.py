...
plt.plot(epochs, history.history['loss'], label='Train loss Loss')

plt.title('Model Validation and Training Loss Over Epochs, T(x) = {}'.format(min_rounds))
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.tight_layout()
plt.savefig('figures/epoch_comparison_lstm_{}.pdf'.format(min_rounds), format='pdf', bbox_inches='tight')